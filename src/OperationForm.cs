...
namespace BitcoinInfoMiner
{
    ...
    public partial class OperationForm : Form
    {
        ...
        //Парсинг Bittrex
        public async Task parseBittrexOperation(double balance,string currency)
        {
            ...
            for (int iterMain = 0; iterMain < wallets.Length; iterMain += 2)
            {
                if (iterMain + 1 >+ wallets.Length)
                    break;
                var mainBodyAll=await Wallets.getBittrexOperation(wallets[iterMain],wallets[iterMain + 1],currency);
                int id = 0;
                ...
                //    testing[itern] = Convert.ToString(dataGridView1.Rows[itern].Cells[5].Value);
                    ...
                    for (int iter = 0; iter < dataGridView1.Rows.Count; iter++)
                    {
                        ...
                        double usdRate = await Wallets.getUSDrateDate(temp * 1000, currency);
        ...
        //Парсинг LTC
        public async Task parseLTCOperation(double balance)
        {
            IList<LTCBody> mainBodyAll = await Wallets.getLTCOperation(wallets);
            int id = 0;
            rowFullInfo = new TransactionInfo[mainBodyAll.Count];
            for (int iter = 0; iter < mainBodyAll.Count; iter++)
            {
            ...
            await Task.Run(async delegate
            {
                for (int iter = 0; iter < dataGridView1.Rows.Count; iter++)
                {
                    ...
                    double usdRate = await Wallets.getUSDrateDate(
                        Wallets.ToUnixFromDateTime((DateTime)dataGridView1.Rows[iter].Cells["Data"].Value)
                        * 1000, "LTC");
        ...
        //Парсинг ETH
        public async Task parseETHOperation(double balance)
        {
                IList<ETHBody> mainBodyAll =await  Wallets.getETHOperation(wallets);
                int id = 0;
                rowFullInfo = new TransactionInfo[mainBodyAll.Count];
                for (int iter = 0; iter < mainBodyAll.Count; iter++)
                {
        ...
        public async Task  parseBlockchainOperation(double balance)
        {
            IList<OperationAllData> mainBody = await Wallets.getBlockchainOperation(wallet);
            int id = 0;
            ...
            await Task.Run(async delegate
            {
                for (int iter = 0; iter < mainBody.Count; iter++)
                {
                    double usdRate = await Wallets.getUSDrateDate(mainBody[iter].time * 1000);
                    dataGridView1.Rows[iter].Cells[7].Value = Math.Round(Convert.ToDecimal(dataGridView1.Rows[iter].Cells[5].Value,
                        CultureInfo.InvariantCulture) * Convert.ToDecimal(usdRate, CultureInfo.InvariantCulture), 2);
                    dataGridView1.Rows[iter].Cells[6].Value = Math.Round(Convert.ToDecimal(dataGridView1.Rows[iter].Cells[4].Value,
                        CultureInfo.InvariantCulture) * Convert.ToDecimal(usdRate, CultureInfo.InvariantCulture), 2);
        ...
        public async Task parseAllBlockchainOperation()
        {

            IList<OperationAllData> mainBodyAll =  await Wallets.getAllBlockchainOperation(wallets);
            int id = 0;
            rowFullInfo = new TransactionInfo[mainBodyAll.Count];
            for (int iter = 0; iter < mainBodyAll.Count; iter++)
            {
            ...
            await Task.Run(async delegate
            {
                for (int iter = 0; iter < mainBodyAll.Count; iter++)
                {
                    double usdRate = await Wallets.getUSDrateDate(mainBodyAll[iter].time * 1000);
        ...
        //Парсинг NiceHash
        public async Task parseNiceHashOperation(double balance)
        {
            IList<NiceHashBody> mainBodyAll = await Wallets.getNiceHashOperation(wallets);
            int id = 0;
            for (int iter = 0; iter < mainBodyAll.Count; iter++)
            {
                dataGridView1.Rows.Add(1);
