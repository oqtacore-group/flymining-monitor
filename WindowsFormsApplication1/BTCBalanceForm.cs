 ...
 class DataRecord
    {
     ...
     public String Closed { get; set; }
        ...
        public async Task createDataGrid()
        {
                    ...
                    case "BTC cold wallet":
                        try
                        {
                            if (Wallets.walletInfo[key].Length > 0)
                            {
                                ...
                                {
                                    parseBlockchain(await Wallets.sendAsyncRequest("q/addressbalance/" + Wallets.walletInfo[key][iter],
                    ...
                    case "LTC":
                        try 
                        {
                            if (Wallets.walletInfo[key].Length > 0)
                            {
                                ...
                                for (int iter = 0; iter < Wallets.walletInfo[key].Length; iter++)
                                {
                                    parseLTC(await Wallets.sendAsyncRequest("address/LTC/" + Wallets.walletInfo[key][iter],
                            ...
                            if (Wallets.walletInfo[key].Length > 0)
                            {
                                ...
                                for (int iter = 0; iter < Wallets.walletInfo[key].Length; iter += 3)
                                {
                                    if (iter + 2 < Wallets.walletInfo[key].Length)
                                    {
                                        ...
                                        parseNiceHash(await Wallets.sendAsyncRequest("", Wallets.NiceHashUri, uriArgs),
                            ...
                            if (Wallets.walletInfo[key].Length > 0)
                            {
                                ...
                                for (int iter = 0; iter < Wallets.walletInfo[key].Length; iter += 2)
                                {
                                    if (iter + 1 < Wallets.walletInfo[key].Length)
                                        parseBittrex(await Wallets.sendBittrexRequest("account/getbalances",
                        ...
                        try
                        {
                            if (Wallets.walletInfo[key].Length > 0)
                            {
                                ...
                                for (int iter = 0; iter < Wallets.walletInfo[key].Length; iter++)
                                {
                                    ...
                                    parseETH(await Wallets.sendAsyncRequest("", Wallets.ethUri, uriArgs), iter, key, dataGridArray[key]);
                                }
                                foreach (DataGridViewColumn column in dataGridArray[key].Columns)
                                {
                                    column.SortMode = DataGridViewColumnSortMode.NotSortable;
        ...
        private void parseBittrex(string json, int walletID, string walletKey, DataGridView grid)
        {
            if (json != "False")
            {
                ...
                else
                {
                    ...
                    grid.Rows[grid.Rows.Count - 4].Cells["Wallet"].Value = Wallets.walletInfo[walletKey][walletID];
                    grid.Rows[grid.Rows.Count - 4].Cells["Description"].Value = Wallets.walletInfoDescription[walletKey][walletID];
                    grid.Rows[grid.Rows.Count - 4].Cells["Button"].Value = "Order History";
                    for (int iter = 0; iter < bittrexResponse.Result.Count(); iter++)
                    {
                        if (!grid.Columns.Contains(bittrexResponse.Result[iter].First.First.ToString()))
                        {
                            ...
                            newCol.HeaderText=bittrexResponse.Result[iter].First.First.ToString();
                            newCol.Name=bittrexResponse.Result[iter].First.First.ToString();
                            newCol.CellTemplate = new DataGridViewTextBoxCell();
                            grid.Columns.Insert(grid.Columns.Count - 2, newCol);
                        }
                ...
                try
                {
                    for (int iterMain = 2; iterMain < grid.Columns.Count - 1; iterMain++)
                    {
                        ...
                        {
                            string cell=(string)grid.Rows[iter].Cells[iterMain].Value;
                            if (cell!="")
                                tempNum += Convert.ToDouble(cell, CultureInfo.InvariantCulture);
                        }
        ...
        {
            //string[] tempAll = new string[2];
            //for (int iter=0;iter<Wallets.walletInfo["Bittrex"].Length;iter+=2)
            //{
            //    if (Wallets.walletInfo["Bittrex"][iter]==row.Cells["Wallet"].Value.ToString())
            //    {
            //        tempAll[0] = Wallets.walletInfo["Bittrex"][iter];
            //        tempAll[1] = Wallets.walletInfo["Bittrex"][iter+1];
            //    }
            //}
        ...
        public void saveOrderHistory(string wallet)
        {
            ...
            using( FileStream stream=new FileStream(fileName,FileMode.Append))
            {
                ...
                {
                    if (Wallets.orderHistory[iter].wallet == wallet)
                    {
        ...
        private void panel1_SizeChanged(object sender, EventArgs e)
        {
            foreach (string key in dataGridArray.Keys)
            {
                ...
                {
                    dataGridArray[key].Width = 800;
                    labelArray[key].Location = new System.Drawing.Point(400 - labelArray[key].Width, labelArray[key].Location.Y);
                    dataGridArray[key].AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
                }
            }
