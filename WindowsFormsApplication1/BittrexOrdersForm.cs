    ...
    public partial class BittrexOrdersForm : Form
    {
        ...
        private async void populateDataGrid()
        {
            for (int iter = 0; iter < Wallets.walletInfo["Bittrex"].Count(); iter ++)
                ...
                {
                     await Wallets.parseOrderHistory(currentWallet, Wallets.walletInfo["Bittrex"][iter+1]);
                }            
            ...
            //    }
            await parseWallets(Wallets.walletInfo["Bittrex"]); 
        }
        public async Task parseWallets(string[] wallets)
        {
        ...
        {
            //string fileName = Application.StartupPath + "//" + bittrexOldOrderHistory + wallet + ".csv";
        ...
        private void buttonCSVImport_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(openFileDialog.FileName) && File.Exists(openFileDialog.FileName) )
            {
                ...
                string fileName = Application.StartupPath + "//" + Wallets.bittrexOldOrderHistory + currentWallet + ".csv";
